...
BEGIN_MESSAGE_MAP(CEditImage, CButton)
	ON_WM_ERASEBKGND()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
void CEditImage::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	for (x = 0; x < m_sizeImage.cx; x++)
	{
		for (y = 0; y < m_sizeImage.cy; y++)
		{
			...
			if (rgbPixel != (COLORREF)-1)
			{
				...
				TRACE(_T("(%02d,%02d) : %03d,%03d,%03d\n"),x,y, GetRValue(rgbPixel), GetGValue(rgbPixel), GetBValue(rgbPixel));
			}
		}
	}
...
BOOL CEditImage::OnEraseBkgnd(CDC* /*pDC*/)
{
	return TRUE;
}
...
void CEditImage::OnLButtonUp(UINT /*nFlags*/, CPoint point)
{
	if (GetCapture() == this)
	{
		ReleaseCapture();
...
void CEditImage::DrawPixel(POINT point)
{
	CRect rectClient; // Client area rectangle
	GetClientRect(&rectClient);
...
void CEditImage::SetBitmap(CBitmap* pBitmap)
{
	m_pBitmap = pBitmap;
...
BOOL CEditImage::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	CPoint ptCursor;
...
void CEditImage::PreSubclassWindow()
{
...
void CEditImage::ScreenToBitmap(CPoint& point)
{
...
void CEditImage::BitmapToClient(CRect& rect)
{
...
void CEditImage::FloodFill(const CPoint& point)
{
...
void CEditImage::SetColor(COLORREF color)
{
	m_rgbColor = color;
